...
#____________________________ MY WORK BEGINS HERE _____________________________#
  ...
  def self.find_by_playwright(name)
    play = PlayDBConnection.instance.execute(<<-SQL, name)
      ...
      FROM
        plays
      JOIN
        playwrights ON playwrights.id = plays.id
      WHERE
        playwrights.name = ?
    SQL
    return nil if play.length <= 0
...
#_________________________ DONE WITH CLASS PLAY _______________________________#
